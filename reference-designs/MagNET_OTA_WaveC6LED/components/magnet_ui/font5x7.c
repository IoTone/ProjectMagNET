/*
 * font5x7 — ASCII 32..90 (space through 'Z'), 5 columns per glyph, one byte per
 * column, bit 0 = top row. Stops at 'Z' on purpose: a status display needs
 * digits, capitals and a little punctuation, and 59 glyphs is 295 bytes against
 * 480 for the full printable set. Lowercase folds to uppercase at draw time.
 */
#include <stdint.h>
