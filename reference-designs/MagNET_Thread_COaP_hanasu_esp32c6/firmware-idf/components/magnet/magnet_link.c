...
static void emit_caps(const char *tag) {
    respond(tag,
        "+OK proto=2.1 fw=0.2.0-eb maxline=512 "
        "transports=usbcdc verbs=STATUS,CAPS,HELP,PING,CHAT,DM,PEERS,WHOAMI,"
        "SYSINFO,MESH,BENCH,SELFTEST,STATS,STRESS,HEARTBEAT,FORTH "
        "events=ready,state,chat,dm,peer,role,heartbeat,warn mode=HCP");
}
...
static void handle_hcp_line(char *line) {
    ...
    if      (!strcmp(verb, "PING"))   respond(tag, "+PONG");
    else if (!strcmp(verb, "STATUS")) { mn_status_print(); respond(tag, "+OK"); }
    else if (!strcmp(verb, "CAPS"))   emit_caps(tag);
    else if (!strcmp(verb, "HELP"))   emit_help(tag);
    else if (!strcmp(verb, "WHOAMI")) { mn_whoami_print(); respond(tag, "+OK"); }
        ...
        if (mn_get_state() != MN_READY) { respond_err(tag, "E_BAD_STATE", mn_state_name(mn_get_state())); return; }
        if (mn_chat(rest, strlen(rest)) != 0) { respond_err(tag, "E_INTERNAL", "send failed"); return; }
        respond(tag, "+OK");
    }
