 ...
 * keys and republish any new value to the single AXR device topic
 * "iotj/axrawe/updates/<DEVICEID>" (matching M5StickCPlus-Blinky_Voice_XR).
 *
...
/* DEVICEID (MAC suffix, %02x%02x of mac[4..5]) — set in derive_ids(). Used to
 * build the single AXR telemetry topic, matching M5StickCPlus-Blinky_Voice_XR. */
static char     mac_suffix[8] = "0000";
...
/* All bridged values publish to the single AXR device topic
 * "iotj/axrawe/updates/<DEVICEID>", matching M5StickCPlus-Blinky_Voice_XR.
 * The hive key is ignored for topic purposes — every value goes to the one
 * device topic (values interleave by tick; per-key dedup still applies). */
static void key_to_topic(const char *key, char *out, size_t out_len) {
    (void)key;
    snprintf(out, out_len, "iotj/axrawe/updates/%s", mac_suffix);
}
...
/* ---------- AXR heartbeat (hive-independent) ----------
 * Publishes a small device-status JSON to iotj/axrawe/updates/<id> on a timer
 * once MQTT is connected — independent of SNTP / hive / bridge. This guarantees
 * traffic on the XR topic even on networks that block NTP (no time sync) or
 * where no hive ruler is present to bridge from. Bridged hive KV values (when
 * the hive is up + bridge enabled) publish to the SAME topic via bridge_tick(). */
#define HEARTBEAT_PERIOD_S   10
static char s_hb_topic[MQTT_BRIDGE_TOPIC_MAX];
static char s_hb_payload[160];
static void publish_heartbeat(void) {
    if (!craw_mqtt_is_connected()) return;
    snprintf(s_hb_topic, sizeof(s_hb_topic), "iotj/axrawe/updates/%s", mac_suffix);
    uint32_t up_s = (uint32_t)(esp_timer_get_time() / 1000000ULL);
    snprintf(s_hb_payload, sizeof(s_hb_payload),
        "{\"id\":\"%s\",\"node\":\"scribe\",\"up\":%lu,\"heap\":%lu,"
        "\"hive\":\"%s\",\"ip\":\"%s\"}",
        mac_suffix, (unsigned long)up_s,
        (unsigned long)esp_get_free_heap_size(),
        craw_hive_node_session_id() ? "joined" : "idle",
        ip_str);
    craw_mqtt_publish(s_hb_topic, s_hb_payload, 0, false);
}

static void housekeeping_task(void *arg) {
    (void)arg;
    /* Resolve hostname + mac_suffix immediately (independent of the SNTP/hive
     * gate in maybe_start_hive) so the heartbeat topic uses the real DEVICEID
     * rather than the "0000" default. */
    derive_ids();
    uint32_t hb_ticks = 0;
    const uint32_t hb_period_ticks = HEARTBEAT_PERIOD_S * 1000 / 250;  /* loop is 250 ms */
    while (1) {
        ...
        /* Hive-independent heartbeat on the AXR topic. */
        if (++hb_ticks >= hb_period_ticks) {
            hb_ticks = 0;
            publish_heartbeat();
        }
...
/* ---------- Live IMU -> MQTT publisher ----------
 * Streams the BMI270 fused orientation to the AXR topic at IMU_PUBLISH_RATE,
 * matching M5StickCPlus-Blinky_Voice_XR's payload + 10 Hz cadence. The IMU
 * sampler is already started at boot by boot_ritual_task (50 Hz), so this just
 * snapshots + publishes. Runs in its own task because the 250 ms housekeeping
 * loop is far too slow for 10 Hz. */
#define IMU_PUBLISH_PERIOD_MS  100   /* 10 Hz */
static char s_imu_topic[MQTT_BRIDGE_TOPIC_MAX];
static char s_imu_json[256];
static void imu_pub_task(void *arg) {
    (void)arg;
    while (1) {
        if (craw_mqtt_is_connected() && craw_imu_is_running()) {
            craw_imu_snapshot_t s;
            craw_imu_snapshot(&s);
            snprintf(s_imu_topic, sizeof(s_imu_topic),
                     "iotj/axrawe/updates/%s", mac_suffix);
            snprintf(s_imu_json, sizeof(s_imu_json),
                "{\"id\":\"%s\",\"ts\":%llu,"
                "\"ax\":%.3f,\"ay\":%.3f,\"az\":%.3f,"
                "\"gx\":%.3f,\"gy\":%.3f,\"gz\":%.3f,"
            ...
            craw_mqtt_publish(s_imu_topic, s_imu_json, 0, false);
        }
        vTaskDelay(pdMS_TO_TICKS(IMU_PUBLISH_PERIOD_MS));
    }
}
...
void app_main(void) {
    ...
    /* Live IMU -> MQTT at 10 Hz (matches Voice_XR). Publishes once the boot
     * ritual has the BMI270 sampling and MQTT is connected. */
    xTaskCreate(imu_pub_task, "imu_pub", 4096, NULL, 4, NULL);
