...
static char *build_hello_payload(void) {
    ...
    if (s_ctx.cfg.gen) cJSON_AddStringToObject(p, "gen", s_ctx.cfg.gen);
...
static void session_attempt(void) {
    ...
    free(frame);
    if (rc != 0) {
        ESP_LOGW(TAG, "decode of first ruler frame failed rc=%d", rc);
    ...
    /* Layer 2 lineage gate: ruler issues CHALLENGE between HELLO and WELCOME
     * when require_lineage_auth is set. We answer with RESPONSE, then read
     * WELCOME on the next frame. Backward compatible — pre-gate rulers go
     * straight to WELCOME and the if-block is skipped. */
    if (in.type == CRAW_HIVE_MSG_CHALLENGE) {
        cJSON *cp = cJSON_Parse(in.payload_json ? in.payload_json : "{}");
        const cJSON *jl  = cJSON_GetObjectItemCaseSensitive(cp, "lineage");
        const cJSON *jp  = cJSON_GetObjectItemCaseSensitive(cp, "puzzle");
        const cJSON *jct = cJSON_GetObjectItemCaseSensitive(cp, "chal_ts");
        if (!cJSON_IsString(jl) || !cJSON_IsString(jp) || !cJSON_IsNumber(jct)) {
            ESP_LOGW(TAG, "malformed CHALLENGE payload");
            cJSON_Delete(cp);
            craw_hive_msg_free(&in);
            close(sock); s_ctx.sock = -1;
            set_state(CRAW_HIVE_NODE_BACKOFF, "bad-challenge");
            return;
        }
        const magnet_lineage_t *lin = magnet_lineage_find(jl->valuestring);
        if (!lin) {
            ESP_LOGW(TAG, "ruler asked for unknown lineage '%s'", jl->valuestring);
            cJSON_Delete(cp);
            craw_hive_msg_free(&in);
            close(sock); s_ctx.sock = -1;
            set_state(CRAW_HIVE_NODE_BACKOFF, "lineage-unknown");
            return;
        }
        char answer[65];
        int64_t chal_ts = (int64_t)jct->valuedouble;
        if (magnet_lineage_compute_response(lin->key, jp->valuestring,
                                             s_ctx.cfg.node_id, chal_ts,
                                             answer, sizeof(answer)) != 0) {
            cJSON_Delete(cp);
            craw_hive_msg_free(&in);
            close(sock); s_ctx.sock = -1;
            set_state(CRAW_HIVE_NODE_BACKOFF, "compute-response");
            return;
        }
        cJSON *rp = cJSON_CreateObject();
        cJSON_AddStringToObject(rp, "lineage", jl->valuestring);
        cJSON_AddStringToObject(rp, "answer",  answer);
        char *rps = cJSON_PrintUnformatted(rp);
        cJSON_Delete(cp);
        craw_hive_msg_free(&in);

        rc = send_msg(sock, CRAW_HIVE_MSG_RESPONSE, "*", rps);
        free(rps); cJSON_Delete(rp);
        if (rc != 0) {
            close(sock); s_ctx.sock = -1;
            set_state(CRAW_HIVE_NODE_BACKOFF, "send-response");
            return;
        }
        ESP_LOGI(TAG, "tx RESPONSE for lineage challenge");

        /* Read again — should be WELCOME (or REJECT on auth fail). */
        frame = NULL; flen = 0;
        if (recv_frame(sock, &frame, &flen) != 0) {
            close(sock); s_ctx.sock = -1;
            set_state(CRAW_HIVE_NODE_BACKOFF, "recv-welcome2");
            return;
        }
        ...
        if (rc != 0) {
            craw_hive_msg_free(&in);
            close(sock); s_ctx.sock = -1;
            set_state(CRAW_HIVE_NODE_BACKOFF, "post-response-decode");
            return;
        }
    }

    if (in.type != CRAW_HIVE_MSG_WELCOME) {
        ESP_LOGW(TAG, "expected WELCOME, got type=%d", (int)in.type);
        craw_hive_msg_free(&in);
        close(sock); s_ctx.sock = -1;
        set_state(CRAW_HIVE_NODE_BACKOFF, "no-welcome");
        return;
