...
static void mqtt_event_handler(void *args, esp_event_base_t base,
    ...
    switch (event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            /* Only subscribe if a base_topic was supplied (publisher-only
             * mode passes NULL/empty and skips subscription). */
            if (s_base_topic[0]) {
                esp_mqtt_client_subscribe(s_client, s_sub_topic, 1);
                ESP_LOGI(TAG, "subscribed to %s", s_sub_topic);
            }
...
void craw_mqtt_init(const char *broker_uri, const char *base_topic,
    ...
    /* base_topic is optional. NULL/empty = publisher-only mode (no
     * subscription on connect). Crawdad uses subscribe; the MagNET MQTT
     * bridge in the Capsule scribe uses publish-only. */
    if (base_topic && base_topic[0]) {
...
int craw_mqtt_publish(const char *topic, const char *payload,
    ...
    return esp_mqtt_client_publish(s_client, topic, payload, 0,
                                   qos, retain ? 1 : 0);
}
