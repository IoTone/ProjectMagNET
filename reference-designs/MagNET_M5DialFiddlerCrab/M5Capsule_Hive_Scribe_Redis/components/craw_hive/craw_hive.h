...
typedef enum {
    ...
    /* Lineage gate — see docs/MagNET-Generations.md "Layer 2".
     * Inserted between HELLO and WELCOME when the ruler enables it. */
    CRAW_HIVE_MSG_CHALLENGE    = 11,
    CRAW_HIVE_MSG_RESPONSE     = 12,
...
typedef enum {
    ...
    /* Lineage gate (Layer 2) — see docs/MagNET-Generations.md. */
    CRAW_HIVE_REJECT_LINEAGE_UNKNOWN  = 6,  /* peer doesn't carry the requested lineage key */
    CRAW_HIVE_REJECT_LINEAGE_AUTH     = 7,  /* RESPONSE HMAC didn't verify */
    CRAW_HIVE_REJECT_GEN_TOO_OLD      = 8,  /* peer gen below ruler's policy floor */
...
typedef struct {
    ...
    const char *gen;        // MAGNET_GEN_STR — e.g. "0.5.0-spore"; NULL = omit field
    ...
    void *on_kv_get_ctx;
    // Lineage puzzle gate (Layer 2). When true, the ruler issues a
    // CHALLENGE between HELLO and WELCOME. The joiner must answer with a
    // RESPONSE keyed by the DNA key for the lineage in its gen tag (see
    // magnet_lineages.h). Default false → no challenge issued (backward
    // compatible). Peers that omit `gen` from HELLO bypass the challenge
    // even when this flag is true; tighten with require_gen if needed.
    bool require_lineage_auth;
    // When true, REJECT any HELLO that omits the `gen` field. Independent
    // of require_lineage_auth — set both to enforce mandatory lineage proof.
    bool require_gen;
...
// Look up a connected peer's gen string (e.g. "0.5.0-spore"). Returns 0
// on hit, -1 if peer not connected. Empty string if peer didn't send gen.
int craw_hive_ruler_peer_gen(const char *node_id, char *out, size_t out_len);

// Runtime control of the lineage puzzle gate. Initial value comes from
// cfg.require_lineage_auth at start; these let a Forth REPL or app code
// flip it without restarting the ruler. Affects only NEW joins — peers
// already in WELCOME stay connected.
void craw_hive_ruler_set_lineage_auth(bool on);
bool craw_hive_ruler_get_lineage_auth(void);
