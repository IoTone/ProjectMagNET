...
- **Button**: GPIO 42 — cycles views: App → Settings → Settings Rotation → App
...
- ~~RFE6: Because the USB port is placed at a particular spot, the display orientation is set such that the north position is opposite the USB cable. We need a setting to allow us to change the orientation and save it.~~ **DONE**: The hardware button is now a 3-way view cycler (App → Settings → Settings Rot → App). The new "Settings Rotation" view is a dedicated full-screen rotation picker — the encoder live-previews the orientation (entire screen reorients instantly on each detent), and the current preview is auto-saved to NVS when the user presses the button to exit (advance to App view). New `display_rot` NVS key (u8, 0-3 for 0°/90°/180°/270°) is loaded at boot via `nvs_load_rotation()` and applied with `display.setRotation()` before the first scene draw. M5GFX's `convertRawXY()` automatically rotates touch coordinates, so the Settings view bounding boxes remain valid at any rotation — no layout changes needed. New REPL words `N rotation` (set) and `rotation?` (get) provide programmatic access. If the user advances through rotation view without actually changing anything, no NVS write happens (no churn, no save chime).
- ~~RFE7: The fiddler crab needs to be a bit more cute and active.~~ **DONE**: The crab now animates per Claude CLI state. `draw_crab()` uses a bounding-box clear + full redraw strategy with `millis()`-driven animation offsets. **WORKING**: 6 legs scurry with alternating ±3px foot shifts every 150ms. **NEED INPUT**: big claw pincers snap open/closed every 400ms. **FINISHED**: small claw tip orbits the base in a 6px-radius circle (~2s/revolution via `sinf`/`cosf`). **IDLE**: eyes blink (close for 150ms every 3s with dim-cyan squint lines) and pupils shift left/right every 4.5s. **ERROR**: static crab (status text already flashes red). Active states redraw every frame; IDLE redraws at ~30fps. Animation constants in `Config` namespace are tunable.