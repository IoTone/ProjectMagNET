...
#### H1 — Converge the Forth core ⬅ start here
...
#### H2 — Rollback in `craw_role_bundle`
...
#### H3 — DECIDE: bundle signature algorithm, then implement as RoleBundle v2
...
#### H4 — Factor the shared verified-apply engine
...
| Vestigial `forth_heap_used() ? 0 : 0` — success log always prints dict `0` | Wave `ota_apply.c:119` |
...
**H1.** Everything else stacks on the converged core: H2 needs `forth_save`, H3's engine
target needs H2, H4 needs both, H5–H8 need H4's engine to have one home. It is also the
lowest-risk item on the list — a strict-superset merge of code already validated on two
chip families, with the FFI-registration shim as the only thing to watch.
