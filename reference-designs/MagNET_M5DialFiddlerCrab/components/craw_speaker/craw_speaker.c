/*
 * craw_speaker.c — ESP-IDF LEDC-based buzzer driver for MagNET Claw devices.
 */
...
void craw_speaker_chime_working(void)
{
    ...
    craw_speaker_tone(800, 50);
}

void craw_speaker_chime_finished(void)
{
    if (!sound_enabled) return;
    craw_speaker_tone(1200, 80);
...
{
    if (!sound_enabled) return;
    craw_speaker_tone(2000, 100);
}

void craw_speaker_chime_error(void)
{
    if (!sound_enabled) return;
    craw_speaker_tone(1000, 80);
    tone2_at   = millis_now() + 120;
    tone2_freq = 600;
    tone2_dur  = 100;
}
