...
static const char *TAG = "claw_wifi";
...
static claw_wifi_event_cb_t s_event_cb  = NULL;
static void                *s_event_ctx = NULL;

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
static void fire_event(claw_wifi_event_t ev)
{
    if (s_event_cb) {
...
static void wifi_retry_task(void *arg)
{
    ...
    if (!wifi_connected) {
        ...
        fire_event(CLAW_WIFI_EVENT_CONNECT_FAILED);
    }
    vTaskDelete(NULL);
}
                                ...
                                int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT) {
        if (event_id == WIFI_EVENT_STA_START) {
            ...
            fire_event(CLAW_WIFI_EVENT_DISCONNECTED);
        ...
        wifi_connected = true;
        wifi_retry_count = 0;
        fire_event(CLAW_WIFI_EVENT_CONNECTED);
    }
}
