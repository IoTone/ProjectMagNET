/**
 * claw_serial.c — Platform-abstracted serial I/O
 *
...
void claw_serial_putchar(int c)
{
    uint8_t ch = (uint8_t)c;
    usb_serial_jtag_write_bytes(&ch, 1, pdMS_TO_TICKS(100));
}

void claw_serial_print(const char *s)
{
...
#define CLAW_UART_NUM    UART_NUM_0
#define CLAW_UART_BUF    256
...
void claw_serial_init(void)
{
    ...
    uart_param_config(CLAW_UART_NUM, &uart_config);
    /* Don't set pins — UART0 uses default TX=1, RX=3 */
    uart_driver_install(CLAW_UART_NUM, CLAW_UART_BUF, 0, 0, NULL, 0);
    uart_installed = true;
}
...
{
    uint8_t c;
    int n = uart_read_bytes(CLAW_UART_NUM, &c, 1, pdMS_TO_TICKS(10));
    if (n <= 0) return -1;
    return (int)c;
}
...
{
    uint8_t ch = (uint8_t)c;
    uart_write_bytes(CLAW_UART_NUM, (const char *)&ch, 1);
}

void claw_serial_print(const char *s)
{
    uart_write_bytes(CLAW_UART_NUM, s, strlen(s));
}
...
void claw_serial_printf(const char *fmt, ...)
{
    char buf[256];
    va_list args;
    va_start(args, fmt);
    ...
    va_end(args);
    claw_serial_print(buf);
}

void claw_serial_read_line(char *buf, int maxlen, bool echo, bool allow_empty)
{
    /* Short delay then drain any stale bytes in the receive buffer. */
    vTaskDelay(pdMS_TO_TICKS(50));
    while (1) {
        int drain = claw_serial_getchar();
        if (drain < 0) break;
    }
