...
export function buildVideoPanel(opts) {
    const { url, type = 'hls', width = 0.4, aspectRatio = 16 / 9, title, autoplay = true, muted = true, frameIntervalMs = 300, } = opts;
    ...
    if (type === 'hls') {
        initHLS(video, url).then(() => { if (autoplay)
            doPlay(); });
    }
    ...
    const border = new THREE.LineSegments(borderGeo, new THREE.LineBasicMaterial({ color: EDGE.link, transparent: true, opacity: 0.7 }));
    ...
    return {
        ...
        dispose() {
            ...
            if (mjpegImg?.parentElement)
                document.body.removeChild(mjpegImg);
...
async function initHLS(video, url) {
    ...
    if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (_, data) => {
            if (data.fatal)
                console.error('[video] HLS fatal error:', data.type, data.details);
        });
