...
export function buildEdgeBundle(treeRoot, graph, opts = {}) {
    ...
    for (const n of allDescs) {
        const theta = n.x;
        const r = n.y;
        positions.push(new THREE.Vector3(Math.cos(theta - Math.PI / 2) * r, Math.sin(theta - Math.PI / 2) * r, 0));
    }
    ...
    for (const leaf of leaves) {
        ...
        txt.anchorY = 'middle';
        txt.position.set(Math.cos(theta - Math.PI / 2) * labelR, Math.sin(theta - Math.PI / 2) * labelR, 0);
        txt.sync();
        g.add(txt);
    }
    ...
    for (let li = 0; li < maxEdges; li++) {
        ...
        const srcLeafName = graphNodeToLeaf.get(typeof link.source === 'string' ? link.source : link.source.id);
        const tgtLeafName = graphNodeToLeaf.get(typeof link.target === 'string' ? link.target : link.target.id);
        if (!srcLeafName || !tgtLeafName || srcLeafName === tgtLeafName)
            continue;
