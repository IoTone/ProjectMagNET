...
export function buildRidgeline(series, opts = {}) {
    const { width = 0.3, rowHeight = 0.04, depthStep = 0.02, palette = [0xff5577, 0xff99cc, 0xcc99ff, 0x66ccff, 0x66ffcc, 0xffcc66], } = opts;
    ...
    series.forEach((row, rIdx) => {
        ...
        const line = new THREE.Line(lineGeo, new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.9 }));
        g.add(line);
        rowData.push({
            samples: row,
            y0, z,
    ...
    function updateRow(rIdx, time) {
        ...
        for (let i = 0; i < numSamples; i++) {
            ...
            if (i < numSamples - 1) {
                ...
                fillPositions[base] = x;
                fillPositions[base + 1] = y0;
                fillPositions[base + 2] = z;
                fillPositions[base + 3] = x;
                fillPositions[base + 4] = y;
                fillPositions[base + 5] = z;
                fillPositions[base + 6] = nextX;
                fillPositions[base + 7] = nextY;
                fillPositions[base + 8] = z;
                // Triangle 2: bottom-left, top-right, bottom-right
                fillPositions[base + 9] = x;
                fillPositions[base + 10] = y0;
                fillPositions[base + 11] = z;
                fillPositions[base + 12] = nextX;
                fillPositions[base + 13] = nextY;
                fillPositions[base + 14] = z;
                fillPositions[base + 15] = nextX;
                fillPositions[base + 16] = y0;
                fillPositions[base + 17] = z;
            }
        }
