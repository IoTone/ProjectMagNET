...
export class XRRig {
    ...
    constructor(renderer, scene, events = {}) {
        this.events = events;
    ...
    makeFingertipSphere() {
        const sphere = new THREE.Mesh(new THREE.SphereGeometry(0.005, 10, 10), new THREE.MeshBasicMaterial({
    ...
    makeReticle() {
        const reticle = new THREE.Mesh(new THREE.SphereGeometry(0.008, 12, 12), new THREE.MeshBasicMaterial({ color: RETICLE_COLOR, depthTest: false, transparent: true, opacity: 1.0 }));
        reticle.renderOrder = 999;
        reticle.visible = false;
        reticle.userData.noHover = true;
        const halo = new THREE.Mesh(new THREE.RingGeometry(0.01, 0.014, 24), new THREE.MeshBasicMaterial({ color: RETICLE_COLOR, depthTest: false, transparent: true, opacity: 0.7, side: THREE.DoubleSide }));
    ...
    onSelectStart(i) {
        ...
        if (slot)
            slot.beam.material.color.set(BEAM_COLOR_PRESS);
        this.events.onSelectStart?.(i);
    }
    onSelectEnd(i) {
        const slot = this.slots[i];
        if (slot)
            slot.beam.material.color.set(BEAM_COLOR_IDLE);
    ...
    update(_frame) {
        ...
        for (const slot of this.slots) {
            ...
            if (!source) {
                ...
                const hit = hits.find(h => !h.object.userData?.noHover && !h.object.isLine);
                if (hit) {
