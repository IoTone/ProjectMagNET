 ...
 */
export function tweenInstanced(
  mesh: THREE.InstancedMesh,
  from: { pos: THREE.Vector3; scale: number }[],
  to: { pos: THREE.Vector3; scale: number }[],
  duration: number,
  easing: (t: number) => number = easeExpOut,
) {
 ...
 */
export function tweenMeshes(
  meshes: THREE.Mesh[],
  from: { pos: THREE.Vector3; scale: THREE.Vector3 }[],
  to: { pos: THREE.Vector3; scale: THREE.Vector3 }[],
  duration: number,
  easing: (t: number) => number = easeExpOut,
) {
  const startTime = performance.now();
 ...
 */
export function tweenInstancedBoxes(
  mesh: THREE.InstancedMesh,
  from: { pos: THREE.Vector3; scale: THREE.Vector3 }[],
  to: { pos: THREE.Vector3; scale: THREE.Vector3 }[],
  duration: number,
