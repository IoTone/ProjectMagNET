...
import { createSlotWheel } from './SlotWheel';
import { TEXT } from '../ui/palette';
export function createJoinPanel(events = {}) {
    const g = new THREE.Group();
    g.name = 'join-panel';
    g.visible = false;
    let currentState = JoinState.ENTERING;
    let activeSlotIndex = -1;
    // Slot values (empty string = unfilled)
    const DEFAULT_CODE = 'AAAAAA';
    const slotValues = DEFAULT_CODE.split('');
    // --- Root panel block ---
    ...
    g.add(footerText);
    // --- Slot wheel ---
    const slotWheel = createSlotWheel();
    ...
    function setSlotChar(slotIndex, char) {
        if (slotIndex < 0 || slotIndex >= SLOT_COUNT)
            return;
        const upper = char.toUpperCase();
        if (!CHAR_SET.includes(upper))
            return;
    ...
    function submit() {
        if (currentState === JoinState.SUBMITTING || currentState === JoinState.ACCEPTED)
            return;
        deactivateSlot();
        const code = getCode();
        if (code.length < SLOT_COUNT) {
            ...
            for (let i = 0; i < SLOT_COUNT; i++) {
                if (slotValues[i].length === 0) {
                    slotBlocks[i].set({ borderColor: new THREE.Color(TEXT.error), borderOpacity: 1.0 });
                }
            }
