...
export function buildDemoScene() {
    ...
    specs.forEach((spec, i) => {
        ...
        const hitPlane = new THREE.Mesh(new THREE.PlaneGeometry(hitW, hitH), new THREE.MeshBasicMaterial({ visible: false, side: THREE.DoubleSide }));
...
function lineDemo() {
    ...
    const chart = new Chart({
        ...
        .x(x).y(y).mark('line').data(data).render();
...
function barDemo() {
    ...
    const chart = new Chart({
        ...
        .x(x).y(y).mark('bar').data(data).render();
...
function scatterDemo() {
    ...
    const chart = new Chart({
        ...
        .x(x).y(y).mark('scatter').data(data).render();
...
function arcDemo() {
    ...
    const chart = new Chart({
        ...
        .x(x).y(y).mark('arc').data([{ x: 0, y: 0 }]).render();
