...
import { hierarchy, cluster, tree as d3tree, treemap as d3treemap, pack as d3pack } from 'd3-hierarchy';
import { Text } from 'troika-three-text';
import type { HNode } from './sampleHierarchy';
import { TEXT } from '../ui/palette';
import { tweenInstanced, easeExpOut } from '../util/tween';

export interface MorphDemo {
  group: THREE.Group;
  tick(): void;
  nextLayout(): void;
  currentType(): string;
...
type LayoutType = 'tree' | 'sunburst' | 'treemap' | 'pack';
const LAYOUT_CYCLE: LayoutType[] = ['tree', 'sunburst', 'treemap', 'pack'];
...
export function buildMorphDemo(rootData: HNode): MorphDemo {
  const g = new THREE.Group();
  g.name = 'morph-demo';

  const hier = hierarchy(rootData).sum(d => (d as any).value ?? 1).sort((a, b) => (b.value! - a.value!));
  ...
  let currentIdx = 0;
  let currentPositions: { pos: THREE.Vector3; scale: number }[] = [];
  let activeTween: { tick(): void; done: boolean } | null = null;

  const SIZE = 0.28;
  const RADIUS = 0.12;

  function computeLayout(type: LayoutType): { pos: THREE.Vector3; scale: number }[] {
    // Re-create hierarchy for each layout since d3 mutates nodes
    const h = hierarchy(rootData).sum(d => (d as any).value ?? 1).sort((a, b) => (b.value! - a.value!));
    const desc = h.descendants();
    const result: { pos: THREE.Vector3; scale: number }[] = [];
    ...
    switch (type) {
      ...
      case 'sunburst': {
        ...
        const byDepth = new Map<number, typeof desc>();
        for (const n of desc) {
      ...
      case 'treemap': {
        d3treemap<HNode>().size([SIZE, SIZE]).paddingInner(0.004)(h as any);
        for (const n of desc) {
          const any = n as any;
          if (any.x0 !== undefined) {
      ...
      case 'pack': {
        d3pack<HNode>().size([SIZE, SIZE]).padding(0.003)(h);
        for (const n of desc) {
          const any = n as any;
          result.push({
  ...
  function rebuildEdges(positions: { pos: THREE.Vector3; scale: number }[]) {
    g.remove(edgeLines);
    edgeGeo.dispose();
  ...
  function morphTo(type: LayoutType) {
