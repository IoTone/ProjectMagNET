...
import type { DataspaceMenu } from './DataspaceMenu';
import type { XRRig, WristInfo } from '../xrRig';

export interface HandMenuOptions {
  menu: DataspaceMenu;
  rig: XRRig;
  headCamera: THREE.Camera;
  handIndex?: number; // 0 = left (default), 1 = right
}
...
export class HandMenu {
  ...
  private menu: DataspaceMenu;
  private rig: XRRig;
  private headCamera: THREE.Camera;
  private handIndex: number;
  private palmUpDetected = false;
  private hideTimer: number | null = null;
  private _headPos = new THREE.Vector3();
  private _toHead = new THREE.Vector3();

  constructor(opts: HandMenuOptions) {
    this.menu = opts.menu;
    this.rig = opts.rig;
