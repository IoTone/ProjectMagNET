...
export class NodeHoverFx {
    ...
    constructor(camera) {
        ...
        this.halo = new THREE.Mesh(new THREE.SphereGeometry(1, 20, 20), new THREE.MeshBasicMaterial({
    ...
    show(worldPos, label, radius = 0.012) {
    ...
    tick() {
        if (!this.target || !this.group.visible)
            return;
