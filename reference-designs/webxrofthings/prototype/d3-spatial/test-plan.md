...
| DEMO04 | UC4 airplane       | `uc4-airplane.json`     | Real-hardware IMU attitude (M5Capsule BMI270 + Madgwick AHRS, served at `/api/v1/sensor/imu` — `IMU_HOST` envvar points the Vite proxy at the device IP) with gyro dead-reckoning at 60 fps + cabin boombox (procedural music PositionalAudio) + cabin HLS display (Big Buck Bunny test stream) + spatial-photo gallery (3× Gaussian splats). HUD switches mutually-exclusive content modes (Flight Info / Music / Video / Photos). See `examples/uc4-airplane.md`. |
