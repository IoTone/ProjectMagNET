...
import { simulatedJapanTemps } from '../src/demo/japanPrefectures';
import { createJapanTempsProvider, type JapanTempsProvider } from './japanTempsAdapter';
import { simulatedKumamotoVehicles, type KumamotoNetwork } from '../src/demo/kumamotoNetwork';
import { createKumamotoVehiclesProvider, type KumamotoVehiclesProvider } from './kumamotoVehiclesAdapter';
...
export const DEFAULT_FIXED_CODES: Record<string, string> = {
  ...
  DEMO05: 'uc5-japan-temps.json',
  DEMO06: 'uc6-kumamoto-transit.json',
};
...
export interface JoinServerOptions {
  ...
  /**
   * UC5: serve /api/v1/geo/japan-temps from the live JMA/Open-Meteo adapter
   * instead of the deterministic simulation. Default FALSE so tests and
   * programmatic embedders stay hermetic (no network); the standalone
   * `npm run server` entrypoint turns it on (opt out with GEO_LIVE=0).
   * The adapter itself still falls back to simulation when offline.
   */
  geoLive?: boolean;
}
...
export function createJoinServer(opts: JoinServerOptions = {}): JoinServer {
  ...
  // ─── UC5 geo feed — Japan prefecture temperatures ──────────────────────
  // opts.geoLive=true (the standalone entrypoint's default) serves the live
  // JMA AMeDAS adapter: cache-by-timestamp (≤6 map fetches/hour), single-
  // flight, Open-Meteo fallback on a 10-min budget floor, deterministic
  // simulation as final fallback — the client renders whatever arrives and
  // badges by `source` ('jma' | 'open-meteo' | 'simulated').
  // geoLive=false (createJoinServer default) is hermetic: simulation only,
  // no network — keeps tests + programmatic embedders offline-safe.
  // Route lives under /api/v1/geo/* (NOT /api/v1/sensor/*) so the Vite
  // catch-all proxies it here without colliding with the real-device
  // /api/v1/sensor → ENV_HOST rule. See docs/uc5-uc6-geo-dataspaces.md §3.1.
  const geoProvider: JapanTempsProvider | null =
    opts.geoLive ? createJapanTempsProvider() : null;
  app.get('/api/v1/geo/japan-temps', async (_req, res) => {
    if (!geoProvider) {
      res.json(simulatedJapanTemps(Date.now()));
      return;
    }
    res.json(await geoProvider.get()); // never rejects — worst case simulated
  });

  // ─── UC6 geo feed — Kumamoto transit vehicles ───────────────────────
  // opts.geoLive=true (standalone entrypoint default): live GTFS-RT from
  // the Bus-Vision open feeds (km.bus-vision.jp vpos, 15 s upstream
  // cadence — the adapter enforces a 15 s cache + single-flight so the
  // publisher sees ONE sweep per window regardless of client count),
  // auto-falling back to the deterministic simulation when the feeds are
  // empty (outside ~06:00–23:00 JST) or unreachable — `source` flips
  // between 'gtfs-rt' and 'simulated' and the scene badge follows.
  // geoLive=false (createJoinServer default): hermetic simulation only.
  // Vehicles walk the REAL route polylines from the committed snapshot
  // (public/maps/…, generated by tools/fetch-kumamoto-network.mjs).
  // Buses only — the tram has no public realtime feed.
  const kumamotoNetworkPath = join(__dirname, '..', 'public', 'maps', 'kumamoto-network.json');
  let kumamotoNetwork: KumamotoNetwork | null | undefined; // undefined = not tried
  function getKumamotoNetwork(): KumamotoNetwork | null {
    if (kumamotoNetwork === undefined) {
      try {
        kumamotoNetwork = JSON.parse(readFileSync(kumamotoNetworkPath, 'utf-8')) as KumamotoNetwork;
      } catch {
        console.warn(`[kumamoto-vehicles] network snapshot missing (${kumamotoNetworkPath}) — run tools/fetch-kumamoto-network.mjs`);
        kumamotoNetwork = null;
      }
    }
    return kumamotoNetwork;
  }
  const kumamotoProvider: KumamotoVehiclesProvider | null =
    opts.geoLive ? createKumamotoVehiclesProvider({ network: getKumamotoNetwork }) : null;
  app.get('/api/v1/geo/kumamoto/vehicles', async (_req, res) => {
    if (kumamotoProvider) {
      res.json(await kumamotoProvider.get()); // never rejects
      return;
    }
    const net = getKumamotoNetwork();
    if (!net) {
      res.json({ updated: new Date().toISOString(), source: 'simulated', vehicles: [] });
      return;
    }
    res.json(simulatedKumamotoVehicles(net, Date.now()));
  });
...
if (isMain) {
  ...
  /* UC5 live upstream is on by default for the standalone dev server only
   * (createJoinServer defaults to hermetic simulation). GEO_LIVE=0 forces
   * simulation — handy for deterministic demos or offline dev. */
  const GEO_LIVE = process.env.GEO_LIVE !== '0';
  const server = createJoinServer({ rotationSeconds: ROTATION_SECONDS, geoLive: GEO_LIVE });
  ...
  console.log(`[join-server] UC5 japan-temps: ${GEO_LIVE ? 'LIVE (JMA → Open-Meteo → simulated); GEO_LIVE=0 to disable' : 'simulated (GEO_LIVE=0)'}`);
  console.log(`[join-server] UC6 kumamoto-vehicles: ${GEO_LIVE ? 'LIVE (GTFS-RT → simulated off-hours); GEO_LIVE=0 to disable' : 'simulated (GEO_LIVE=0)'}`);
